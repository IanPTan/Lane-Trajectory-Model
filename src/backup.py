...
def show_path(img, paths, alpha=0.1, stroke=4):
    colors = [
        (0.8, 0.8, 0.8, 1), # A light grey
        (0.6, 0.6, 0.6, 1), # A mid-grey
    ]
    cmap = LinearSegmentedColormap.from_list('my_greys', colors)
    plt.imshow(img[0][:128], origin="lower", cmap=cmap, vmin=0, vmax=1)

    for path in paths:
        waypoints = np.cumsum(np.append(np.zeros(2), path).reshape(-1, 2), axis=-2) * 10 + np.array([64, 0])
        plt.plot(*waypoints.T, color="#03d3fc", alpha=alpha, linewidth=stroke)
...
def isolate(feature_map, seed_point=None, thresh=0.3):
    ...
    if seed_point == None:
        ...
        while feature_map[*seed_point[::-1]] < thresh:
            seed_point[1] -= 1
...
def preprocess(img_matrix, final_width=30, final_size=(256, 128)):
    ...
    left, center, right = h_pinch(img_matrix)
    width = right - left
    ...
    #print(final_x, shared_w + final_x, scaled_x, shared_w + scaled_x)
    #print(final_img.shape, scaled_img.shape)
    #print(final_img[:scaled_h, final_x:shared_w + final_x].shape, scaled_img[:final_h, scaled_x:shared_w + scaled_x].shape, "\n")
    final_img[:scaled_h, final_x:shared_w + final_x] = scaled_img[:final_h, scaled_x:shared_w + scaled_x] / 255
    
    return final_img


def pred_path(images, twin, sal, horizon=195, ratio=0.09, map_shape=(400, 800)):
...
if __name__ == "__main__":
    ...
    #input_imgs, paths = pred_path(images, twin, lambda i: sal(i, 100).transpose(1, 0, 2))
    input_imgs, paths = pred_path(images, twin, lambda i: sal.mean(i)[:, None, :], 260, 0.4, (400, 400))
